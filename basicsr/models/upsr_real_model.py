...
class UPSRRealModel(SRModel):
    ...
    def optimize_parameters(self, current_iter):  
        ...
        # 🔥 初始化约束损失项
        if self.use_learnable_mapping and self.uncertainty_mapper is not None:
            loss_dict['l_guidance'] = 0
            loss_dict['l_contrast'] = 0
            loss_dict['l_reg'] = 0
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            if self.use_learnable_mapping and self.uncertainty_mapper is not None:
                ...
                if guidance_weight > 0:
                    with torch.no_grad():
                        # ✅ 1. 准备 Ground Truth：基于真实GT的预测误差（训练专用）
                        diff_gt = (micro_sr_mse - micro_gt) / 2  # 真实预测误差
                        diff_gt_abs = torch.abs(diff_gt)  # [B, C, H, W]
                        diff_smooth = F.avg_pool2d(diff_gt_abs, kernel_size=3, stride=1, padding=1)
                        
                        # ✅ 使用与base_weight相同的归一化方式（保持一致性）
                        un_max = self.opt['diffusion'].get('un', 1.0)
                        # 截断误差并归一化到[0,1]
                        target_map = torch.clamp(diff_smooth, 0, un_max) / un_max
                        
                        # ✅ 2. 准备对比学习的mask
                        # 将图片拉成直线便于排序
                        flat_diff = diff_gt_abs.view(diff_gt_abs.size(0), -1)
                        # 计算出10%像素的数量
                        k_hard = int(flat_diff.size(1) * 0.1)  # Top 10% 困难区域
                        
                        # 找出误差最大的10%，形成1，0的掩码图
                        hard_threshold = torch.topk(flat_diff, k_hard, dim=1)[0][:, -1:]
                        hard_mask = (diff_gt_abs >= hard_threshold.view(-1, 1, 1, 1)).float()
                        # 找出误差最小的10%
                        easy_threshold = torch.topk(flat_diff, k_hard, dim=1, largest=False)[0][:, -1:]
                        easy_mask = (diff_gt_abs <= easy_threshold.view(-1, 1, 1, 1)).float()
                        
                        # ✅ 3. 计算自适应margin
                        # 真实误差图
                        target_hard = (target_map * hard_mask).sum() / (hard_mask.sum() + 1e-8)
                        target_easy = (target_map * easy_mask).sum() / (easy_mask.sum() + 1e-8)
                        # 计算两者差距作为margin
                        adaptive_margin = (target_hard - target_easy) * 0.5  # 真实差距的50%
                        adaptive_margin = torch.clamp(adaptive_margin, min=0.1, max=0.4)
                    
                    # ✅ 4. Loss A: 对比损失（相对排序）
                    hard_mean = (micro_uncertainty * hard_mask).sum() / (hard_mask.sum() + 1e-8)
                    easy_mean = (micro_uncertainty * easy_mask).sum() / (easy_mask.sum() + 1e-8)
                    
                    loss_contrast = F.relu(adaptive_margin - (hard_mean - easy_mean))
                    
                    # ✅ 5. Loss B: 弱回归损失（防止坍缩）
                    loss_reg = F.smooth_l1_loss(micro_uncertainty, target_map)
                    
                    # ✅ 6. 标准化后加权
                    # 对比损失标准化到[0,1]
                    loss_contrast_norm = loss_contrast / 0.4  # 除以最大margin
                    
                    # 组合损失（alpha=1.0, beta=0.3）
                    alpha = train_opt.get('contrast_weight', 1.0)
                    beta = train_opt.get('regression_weight', 0.3)
                    
                    guidance_loss = alpha * loss_contrast_norm + beta * loss_reg
                    
                    # 记录到loss_dict
                    loss_dict['l_guidance'] += guidance_loss / num_grad_accumulate
                    loss_dict['l_contrast'] = loss_contrast_norm / num_grad_accumulate  # 用于监控
                    loss_dict['l_reg'] = loss_reg / num_grad_accumulate  # 用于监控
                    
                    # 可选：记录margin值（用于调试）
                    if torch.rand(1).item() < 0.01:  # 1%概率打印
                        logger = get_root_logger()
                        logger.info(f"[Guidance] adaptive_margin={adaptive_margin.item():.3f}, "
                                   f"hard_mean={hard_mean.item():.3f}, easy_mean={easy_mean.item():.3f}, "
                                   f"contrast_loss={loss_contrast.item():.4f}, reg_loss={loss_reg.item():.4f}")
                else:
                    guidance_loss = 0
