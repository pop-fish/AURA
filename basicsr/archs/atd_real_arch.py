...
class ATDTransformerLayer(nn.Module):
    ...
    def forward(self, x, td, x_size, params):
        ...
        x = shortcut + (x_win.view(b, n, c) + x_atd + x_aca) / np.sqrt(3)
        # FFN
        x = x + self.convffn(self.norm2(x), x_size)

        b, N, c = x.shape
        b, n, c = td.shape
        # print(x.mean(), td.mean())
...
class ATDB(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=0, embed_dim=dim, norm_layer=norm_layer)
